...
std::istream & operator>>( std::istream & stream, Book & book )
{
  ...
  stream >> std::ws >> std::quoted( workingItem._isbn ) >> std::ws >> delimiter
      >> std::ws >> std::quoted( workingItem._title ) >> std::ws >> delimiter
      >> std::ws >> std::quoted( workingItem._author ) >> std::ws >> delimiter
